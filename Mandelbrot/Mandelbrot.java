...
public class Mandelbrot extends Thread {
    ...
    final static Picture image = new Picture(taille, taille); // Il y a donc taille*taille pixels blancs ou gris à
                                                              // déterminer
    final static int max = 90_000; // C'est le nombre maximum d'itérations pour déterminer la couleur d'un pixel
    ...
    public void run() {
        ...
        for (int i = 0; i < taille; i++) {
            for (int j = (this.zone)*taille/4; j < (this.zone+1)*taille/4; j++) {
                colorierPixel(i, j);
            }
    ...
    // La fonction colorierPixel(i,j) colorie le pixel (i,j) de l'image en gris ou
    // blanc
    ...
    public static boolean mandelbrot(double a, double b, int max) {
        ...
        for (int t = 0; t < max; t++) {
            if (x * x + y * y > 4.0)
                return false; // Le point (a,b) est blanc
            double nx = x * x - y * y + a;
            double ny = 2 * x * y + b;
            x = nx;
            y = ny;
