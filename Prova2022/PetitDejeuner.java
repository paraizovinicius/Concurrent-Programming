...
public class PetitDejeuner {
  public static void main(String[] args) {
    ...
    for (int i = 0; i < nbHobbits; i++)
      new Hobbit(nom[i], table).start();
  }
}
...
class Table { // Les trois Hobbits mangent que quand tous les trois ont faim
  ...
  public synchronized void sAsseoir() {
    ...
    while (nbSentados != nbHobbits) { // a função só vai não-esperar quando nbSentados for igual a 0
      try {
        wait();
      } catch (Exception e) {
        e.printStackTrace();
